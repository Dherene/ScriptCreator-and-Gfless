...
from PyQt5.QtWidgets import QApplication, QScrollArea, QWidget, QMenuBar, QSystemTrayIcon, QVBoxLayout, QGridLayout, QTextEdit, QPlainTextEdit, QLabel, QComboBox, QLineEdit, QSizePolicy
from PyQt5.QtGui import QFontMetricsF, QTextFormat, QPalette, QPainter, QIcon, QTextCursor
...
class loadFull(QDialog):
    def __init__(self, players, text_editors, folder_path):
        ...
        for subfolder in subfolders:
            ...
            add_character_button = QPushButton("+")
            add_character_button.clicked.connect(self.addCharacter)
            self.setup_widgets.append([setup_name_label, add_character_button])
        ...
        self.main_layout.addWidget(setup_name_label, 0, 0, 1, 1)
        self.main_layout.addWidget(selected_chars_label, 0, 1, 1, 2)
    ...
    def addCharacter(self):
        for widget_row in self.setup_widgets:
            if self.sender() in widget_row:
                plus_button = widget_row[-1]
                widget_row.pop(-1)
                characters_combobox = QComboBox()
                characters_combobox.addItems(self.character_names)
                minus_button = QPushButton("-")
                minus_button.clicked.connect(self.removeCharacter)
                minus_button.setMaximumWidth(30)
                minus_button.setMinimumWidth(30)
                widget_row.append(characters_combobox)
                widget_row.append(minus_button)
                widget_row.append(plus_button)
                self.build()
                break
    ...
    def confirmLoadFullScript(self):
        for i in range(len(self.players)):
            ...
            self.players[i][0].periodical_packet_conditions = []
...
""")
        ...
        try:
            for i in range(len(self.setup_widgets)):
                for j in range(1, len(self.setup_widgets[i])-1, 2):
                    index = self.setup_widgets[i][j].currentIndex()
                    ...
                    for k in range(len(conditions_txt_files)):
                        ...
                        with open(cond_path, 'r') as file:
                            ...
                            if cond_type == "recv_packet":
                                ...
                                self.players[index][0].periodical_conditions.append([base_name, script, running_bool, 1])
    ...
    def build(self):
        plus_row = 1
        for i in range(len(self.setup_widgets)):
            self.main_layout.addWidget(self.setup_widgets[i][0], i+plus_row, 0)
            for j in range(1, len(self.setup_widgets[i])-1, 2):
                self.main_layout.addWidget(self.setup_widgets[i][j], i+plus_row, 1)
                self.main_layout.addWidget(self.setup_widgets[i][j+1], i+plus_row, 2)
                plus_row += 1
            self.main_layout.addWidget(self.setup_widgets[i][-1], i+plus_row, 1, 1, 2)
        ...
        self.main_layout.addWidget(self.confirm_button, i+plus_row+1, 0, 1, 3)

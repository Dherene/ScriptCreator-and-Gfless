...
class AddGroupMembersDialog(GroupScriptDialog):
    ...
    def __init__(
        ...
        existing_names = list(existing_member_names or [])
        available_names = list(available_member_names or [])
        existing_group = {
        ...
        super().__init__(
            ...
            member_path,
            group_id,
            [getattr(leader_obj, "name", "")],
            max_total_members,
            mode="extend",
            existing_group=existing_group,
            available_member_names=available_names,
...
class MyWindow(QMainWindow):
    ...
    def add_players_to_current_group(self):
        ...
        current_total = 1 + len(existing_member_names)
        if current_total >= self.group_script_max_members:
            QMessageBox.information(
                self,
                "Group Script Setup",
        ...
        dlg = AddGroupMembersDialog(
            self.players,
            self.text_editors,
            leader_path or self.group_leader_setup_path,
            member_path,
            group_id,
            leader_obj,
            existing_member_names,
